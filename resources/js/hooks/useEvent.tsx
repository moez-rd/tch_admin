...
export function useEvent() {
    ...
    const handleDelete = (e: React.MouseEvent<HTMLAnchorElement, MouseEvent> | React.KeyboardEvent<HTMLAnchorElement> | React.MouseEvent<HTMLButtonElement, MouseEvent>, event: Event<Seminar | Competition>) => {
        e.preventDefault()
        ...
        onConfirm: () => {
            router.delete(route('events.destroy', {id: event.id}), {preserveScroll: true})
        }
    })
    }
        ...
        return competitions.map((competition) => {
            return {
                ...
                information: [
                    ...
                    Number(competition.eventable?.max_participants) === 1 ? "Individual" : `Maks ${Number(competition.eventable?.max_participants)} peserta`,
                    `${Number(competition.event_registrations_count) || 0} pendaftaran`
                ],
                badges: [
                    {
                ...
                },
                {
                    ...
                    linkProps: {
                        href: "",
                        onClick: (event: React.MouseEvent<HTMLAnchorElement, MouseEvent> | React.KeyboardEvent<HTMLAnchorElement>) => handleDelete(event, competition)
                    }
                }
            ]
        }
    ...
    return seminars.map((seminar) => {
        return {
            ...
            information: [
                ...
                `${Number(seminar.event_registrations_count) || 0} pendaftaran`
            ],
            badges: [
                {
            ...
            menu: [
                ...
                },
                {
                    ...
                    linkProps: {
                        href: "",
                        onClick: (event: React.MouseEvent<HTMLAnchorElement, MouseEvent> | React.KeyboardEvent<HTMLAnchorElement>) => handleDelete(event, seminar)
                    }
                }
            ]
        }
...
eventDisplay.push({
    ...
    data: [
        {
            key: "Milestones",
            value: <DescriptionChildList data={getMilestoneDisplayChild(event.milestones!)}/>
        }
    ]
})
...
eventDisplay.push({
    ...
    data: [
        {
            key: "Narahubung",
            value: <DescriptionChildList data={getContactPersonDisplayChild(event.contact_persons!)}/>
        }
    ]
})
